...
// The tag set is based on DeepDanbooru's tag set, consistent with the model's output.

inline constexpr std::array<std::string_view, 9176> tagSet = {"0_0",
                                                              "1boy",
                                                              "1girl",
                                                              "1koma",
                                                              "1other",
...
// helper function to convert a vector of bools to a vector of bits
...
// Model-specific inference logic implementation.
...
private:
    ...
    const char* inputName = "input_1";
    std::vector<int64_t> inputShape = {1, 512, 512, 3};
    const char* tagOutputName = "activation_172";
    size_t tagOutputDim = 9176;
    const char* hashOutputName = "hash_output";
...
DefaltAutoTagger::DefaltAutoTagger(const std::filesystem::path& modelPath)
    ...
    try {
        ...
        if (dmlPresent) {
            ...
            isGpuAvailable = true;
            log("ONNXRuntime: DML execution provider enabled.");
        } else {
            log("ONNXRuntime: DML execution provider not available, fallback to CPU.");
        }
    } catch (const Ort::Exception& e) {
        std::stringstream ss;
        ss << "ONNXRuntime: DML not available, fallback to CPU. Reason: " << e.what();
        log(ss.str());
    }
}
...
bool DefaltAutoTagger::loadModel() {
    try {
        ...
        std::stringstream ss;
        ss << "ONNXRuntime: Failed to load model from " << modelPath << ". Reason: " << e.what();
        log(ss.str());
        return false;
    }
    return true;
...
std::vector<std::pair<std::string, bool>> DefaltAutoTagger::getTagSet() {
    ...
    for (size_t i = 0; i < systemTagStartIndex; ++i) {
        result.emplace_back(tagSet[i], i >= characterTagStartIndex);
    }
...
PredictResult DefaltAutoTagger::predict(const std::vector<float>& inputTensorVec) {
    if (ortSession == nullptr) {
        if (!loadModel()) {
            log("ONNXRuntime: Model not loaded and failed to load.");
    ...
    const char* outputNames[] = {tagOutputName, hashOutputName};
